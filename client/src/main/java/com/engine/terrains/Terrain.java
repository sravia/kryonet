...
public class Terrain {
    ...
    public float getHeightOfTerrain(float worldX, float worldZ) {
        ...
        if (xCoord <= (1 - zCoord)) {
            answer = Math.barryCentric(new Vector3f(0, heights[gridX][gridZ], 0), new Vector3f(1,
                    heights[gridX + 1][gridZ], 0), new Vector3f(0,
                    heights[gridX][gridZ + 1], 1), new Vector2f(xCoord, zCoord));
        } else {
            answer = Math.barryCentric(new Vector3f(1, heights[gridX + 1][gridZ], 0), new Vector3f(1,
    ...
    private RawModel generateTerrain(Loader loader, String heightMap) {
        ...
        try {
            image = ImageIO.read(new File(Config.RESOURCE_PATH + heightMap + ".png"));
        } catch (IOException e) {
            e.printStackTrace();
        }
